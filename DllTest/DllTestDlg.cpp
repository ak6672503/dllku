...
{
	HINSTANCE hInst;
	//hInst = LoadLibrary(L"Dll2.dll");//动态加载DLL
	//typedef int (*ADDPROC)(int a, int b); //定义函数指针类型
	hInst = LoadLibrary(L"Dll3.dll");//动态加载DLL
	typedef int (*ADDPROC)(int a, int b); //定义函数指针类型

	
	//ADDPROC Add = (ADDPROC)GetProcAddress(hInst, "?add@@YAHHH@Z");//获取DLL到处函数
	
	ADDPROC Add = (ADDPROC)GetProcAddress(hInst, MAKEINTRESOURCEA(1));//获取DLL到处函数
